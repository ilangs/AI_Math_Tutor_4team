...
> **AI Agent 과정 팀 프로젝트 | 4팀**
> 초등학교 3학년 학생을 위한 AI 기반 1:1 수학 튜터링 서비스
> LangChain · LangGraph · FastAPI · ChromaDB · OpenAI
...
루미(Lumi)는 초등학교 3학년 학생의 수학 학습을 돕는 AI 튜터 캐릭터입니다.
...
- **단원 선택**: CSV 데이터셋에서 추출한 실제 3학년 수학 단원 목록 제공
- **AI 개념 설명**: OpenAI가 루미 캐릭터로 해당 단원 개념을 초등학생 눈높이에 맞게 설명
- **보충 설명**: "다시 설명해줘" 버튼으로 더 쉬운 추가 설명 요청 가능
- **이해도 평가**: 학생이 자신의 말로 개념을 설명하면 AI가 [PASS] / [FAIL] 판정 + 격려 피드백 제공
- **문제 풀기**: 선택 단원에서 무작위 문제 1개 출제
- **AI 채점**: 학생 답변을 OpenAI가 채점하고 [정답] / [오답] + 상세 풀이 피드백 제공
...
```
학생 질문 입력 → 수학 질문 분류 → ChromaDB RAG 검색 → OpenAI 맞춤 답변
```
...
└────────┬───────────────────────────────┬────────────────────────┘
         ...
         ▼                               ▼
┌─────────────────┐         ┌────────────────────────────────────┐
│  OpenAI OpenAI  │         │         Two-Track 데이터 조회      │
│  (LLM + TTS)    │         │                                   │
...
| **LLM** | OpenAI OpenAI | latest | 고성능 추론, 초등 눈높이 맞춤 설명, 한국어 우수 |
...
```bash
git clone https://github.com/ilangs/AI_Math_Tutor_4team
```
