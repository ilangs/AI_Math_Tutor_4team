...
explain_prompt = ChatPromptTemplate.from_messages([
    ...
    3. 1개의 문장이 모달 화면 폭의 60%를 넘어가는 경우 마침표(.) 뒤에서는 반드시 줄바꿈 하세요.
...
reexplain_prompt = ChatPromptTemplate.from_messages([
    ...
    1개의 문장이 모달 화면 폭의 60%를 넘어가는 경우 마침표(.) 뒤에서는 반드시 줄바꿈 하세요.
...
concept_eval_prompt = ChatPromptTemplate.from_messages([
    ...
    5. 1개의 문장이 모달 화면 폭의 60%를 넘어가는 경우 마침표(.) 뒤에서는 반드시 줄바꿈 하세요.
    6. 수학과 무관한 엉뚱한 대답을 하면 부드럽게 수학 학습으로 유도해 주세요.
    """),
...
answer_eval_prompt = ChatPromptTemplate.from_messages([
    ...
    6. 1개의 문장이 모달 화면 폭의 60%를 넘어가는 경우 마침표(.) 뒤에서는 반드시 줄바꿈 하세요. 
